...
import { getSalesByClientsAndGoodsReport } from "@/lib/reports/service";
...
export async function GET(request: Request) {
  try {
    ...
    const data = await getSalesByClientsAndGoodsReport({
    ...
    const summary = data.reduce(
      (acc, item) => ({
        sum: acc.sum + item.aggregates.sum,
        discount: acc.discount + item.aggregates.discount,
      }),
      { sum: 0, discount: 0 },
    );
